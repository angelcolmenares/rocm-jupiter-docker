...
echo "6. Shell:          docker exec -it rocm7.2-pytorch-jupyter bash"
echo "7. Check GPU:      docker exec rocm7.2-pytorch-jupyter rocm-smi"
echo "8. Backup:         tar -czf backup_$(date +%Y%m%d).tar.gz workspace/"
echo "9. Clean Docker:   docker system prune -a"
echo 'A. Dockser stats:  docker stats rocm7.2-pytorch-jupyter --no-stream --format "table {{.Container}}\t{{.CPUPerc}}\t{{.MemUsage}}{{.NetIO}}\t{{.BlockIO}}"'
echo 'B. Docker rmi   :  docker rmi rocm7.2-pytorch-jupyter:latest 2>/dev/null || true' 
echo ""
